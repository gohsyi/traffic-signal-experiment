...
class Output:
    ...
    def save_model(self, real=True, path=None):

        if not path:
            path = self.output_path
        save_path = self.saver.save(self.sess, path + 'temp.ckpt')
        self.write('[Save] Model saved in %s' % save_path)
        if real:
            onlyfiles = [f for f in listdir(path) if isfile(join(path, f)) and 'temp.ckpt.' in f]
            for f in onlyfiles:
                shutil.copyfile(path + f, path + 'model.ckpt.%s' % f[10:])
            self.write('[Save] Model copied to %s' % (path + 'model.ckpt'))
        return save_path
